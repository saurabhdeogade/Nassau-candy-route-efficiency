...
fig = px.bar(
    chart_df, x="Avg_Lead_Time", y="Route", orientation="h",
    color="Factory",
...
leaderboard = filtered.sort_values("Route_Efficiency_Score", ascending=False)[
    ["Efficiency_Rank","Route","Route_Volume","Avg_Lead_Time",
     "Delay_Frequency_Pct","Route_Efficiency_Score"]
].rename(columns={
...
with col1:
    ...
    for _, row in top5.iterrows():
        st.write(f"**{row['Route']}** -- {row['Avg_Lead_Time']:.0f} days avg, "
                f"{row['Route_Volume']} shipments")
with col2:
