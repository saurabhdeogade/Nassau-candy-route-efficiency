...
if selected_region != "All":
    state_options = sorted(order_df[order_df["Region"] == selected_region]["State/Province"].unique())
else:
...
lt_threshold = st.sidebar.slider("Show only orders with Lead Time above (days)", 0, max_lt, 0)
...
st.write(f"Showing **{len(filtered):,}** of {len(order_df):,} total shipments matching your filters.")
...
if selected_state != "All":
    ...
    by_factory = filtered.groupby("Factory").agg(
        Shipments=("Order ID", "count"), Avg_Lead_Time=("Lead Time", "mean")
    ).reset_index()
...
timeline_fig = px.scatter(
    ...
    title="Individual Shipments Over Time",
    opacity=0.6,
)
...
with st.expander("View raw filtered data"):
    ...
    st.download_button("Download filtered data as CSV", csv, "filtered_shipments.csv", "text/csv")