...
st.caption(f"Showing {len(us_only)} US states/territories on the map. "
           f"{len(non_us)} non-US provinces excluded (see note below).")
...
if len(non_us) > 0:
    with st.expander("Non-US provinces excluded from map (data quality note)"):
        st.dataframe(non_us[["State/Province","Shipment_Count","Avg_Lead_Time"]], hide_index=True)
...
fig2 = px.bar(
    region_summary, x="Region", y="Avg_Lead_Time", color="Total_Shipments",
